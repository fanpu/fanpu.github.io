...
`chunk_gated_delta_rule` (`fla` 0.5.2, `sm_121`) matches a sequential reference
to within a few multiples of working-precision roundoff in bf16 and TF32,
forward and backward, for $T \le 8192$ and $d \in \{64, 128\}$. In bf16 training
the kernel adds no error beyond what every other bf16 matmul in the model adds,
so kernel numerics are not a confound. Also note that fp32 inputs silently use
TF32.


## Future work

There's still a couple of things that I haven't tested but probably should:
